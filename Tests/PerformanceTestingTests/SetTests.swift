...
class SetTests: XCTestCase {
    ...
    func testIsEmpty() {
        let benchmark = Benchmark.nonMutating(setup: makeSet) { _ = $0.isEmpty }
        assertPerformance(.constant, of: benchmark)
    }

    // `count` should be constant-time in the number of elements
    func testCount() {
        let benchmark = Benchmark.nonMutating(setup: makeSet) { _ = $0.count }
        assertPerformance(.constant, of: benchmark)
    }

    // `first` should be constant-time in the number of elements
    func testFirst() {
        let benchmark = Benchmark.nonMutating(setup: makeSet) { _ = $0.first }
        assertPerformance(.constant, of: benchmark)
    }
    ...
    func testContainsHit() {
        let benchmark = Benchmark.nonMutating(setup: makeSet) {
    ...
    func testContainsMiss() {
        let benchmark = Benchmark.nonMutating(setup: makeSet) { _ = $0.contains($0.count) }
        assertPerformance(.constant, of: benchmark)
    }
    ...
    func testInsert() {
        let benchmark = Benchmark.mutating(setup: makeSet) {
    ...
    func testFilter() {
        let benchmark = Benchmark.mutating(setup: makeSet) { _ = $0.filter { $0 % 5 == 3 } }
    ...
    func testRemoveHit() {
        let benchmark = Benchmark.mutating(setup: makeSet) { $0.remove(Int.random(in: 0..<$0.count)) }
        assertPerformance(.constant, of: benchmark)
    }
    ...
    func testRemoveMiss() {
        let benchmark = Benchmark.mutating(setup: makeSet) { $0.remove($0.count) }
        assertPerformance(.constant, of: benchmark)
    }

    // `removeFirst` should be constant-time in the number of elements
    func testRemoveFirst() {
        let benchmark = Benchmark.mutating(setup: makeSet) { $0.removeFirst() }
        assertPerformance(.constant, of: benchmark)
    }
